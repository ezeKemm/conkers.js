...
export async function execute(interaction: CommandInteraction) {
	...
	const reminderRecords: ReminderRecord[] = data.map((day, i) => {
		schedule += `${day.name}: ${day.user ? day.user : day.role}\n`
		const time = ithDate(i + 1)
	...
	for (const [i, _] of reminderRecords.entries()) {
		await scheduleReminder(interaction.client, recordResponse[i])
