...
export const data = new SlashCommandBuilder()
	...
	.addStringOption((option) =>
		option.setName('hours').setDescription('Set hour'),
	)
	.addStringOption((option) =>
		option.setName('minutes').setDescription('Set minute'),
	)
	.addStringOption((option) =>
		option.setName('seconds').setDescription('Set seconds'),
	)

export async function execute(interaction: CommandInteraction) {
	// Type declaration to get access to sub-type methods
	const _interaction = interaction as ChatInputCommandInteraction

	const userId = _interaction.user.id
	const name: string = _interaction.options.getString('name')!
	const channelId = _interaction.channelId

	const when: string = _interaction.options.getString('when')!
	const hour = _interaction.options.getString('hours')
	const minute = _interaction.options.getString('minutes')
	const second = _interaction.options.getString('seconds')

	const iso_string: string =
		hour && minute && second
			? when + `T${hour}:${minute}:${second}.000-04:00`
			: when
	...
	const response = await createReminder(userId, channelId, name, remindAt)
	if (response.status) {
		await _interaction.editReply(`Error! Reminder could not be created.`)
		return
	}
	console.log('response: ', response)
	await _interaction.editReply(
		`Success! Reminder set :: \`${name}\` for <@${userId}> at <t:${Math.floor(remindAt / 1000)}:f>`,
	)

	await scheduleReminder(_interaction.client, response.id)
}
